...
GOOGLE_CUSTOM_API = config.GOOGLE_CUSTOM_API 
...
class ChitChat(BotPlugin):
    ...
    def send_from_messages(self, message_list):
        ...
        for msg in message_list:
            ...
            if mMessage.is_group:
                self.send(mMessage.frm.room, msg)
            else:
                self.send(mMessage.frm, msg)
    ...
    def extractKeyword(self, message_string):
        ...
        param_q = param_q.replace('gogo_rebirth_bot', '');
    ...
    def searchResult(self, message_string):
        ...
        if (not '@gogo' in message_string) and (random.random() <= 0.95):
            print '**** send search result FAIL: not for gogobot and random number is less 0.95 ****'
            return False

        if ('@gogo' in message_string) and ('你' in message_string) and ('怎麼' in message_string) and ('在' in message_string):
            self.send_from_messages(['嚴格來說，我也是開發者啊'])
            return True

	domain = GOOGLE_CUSTOM_API
    ...
    def showWeahterForcast(self):
        weatherToday = json.loads(
            requests.get('http://api.openweathermap.org/data/2.5/forecast?q=taipei&lang=zh_tw&appid=' + WEATHER_API_KEY).content)
        ...
        if u"雨" in weatherToday['list'][forcastTimeZone]['weather'][0]['description'] or u"多雲" in \
            ...
            rainNoti = ['記得提醒@duo大大帶傘', '出門記得帶把傘', '記得要帶傘出門歐！', '出門記得帶把傘',
                        '記得要帶傘出門歐！', '出門記得帶把傘', '記得要帶傘出門歐！', '出門記得帶把傘', '記得要帶傘出門歐！']
            if mMessage.is_group:
                self.send(mMessage.frm.room, random.choice(rainNoti))
            else:
                self.send(mMessage.frm, random.choice(rainNoti))
    ...
    #
		...
		if mMessage.is_group:
		    self.send(mMessage.frm.room, random.choice(response))
		else:
		    self.send(mMessage.frm, random.choice(response))
            else:
                if random.randrange(0, 101) > 80:
                    ...
                    for msg in self.histMsg:
		        if mMessage.is_group:
		            self.send(mMessage.frm.room, random.choice(self.histMsg))
		        else:
		            self.send(mMessage.frm, random.choice(self.histMsg))
                        i += 1
                        if i > 7:
                            return True
                return samePerson
        ...
        for key in badEnding:
            if message_string[idx] == key:
                ...
                if random.randrange(0, 101) > 50:
                    ...
                    self.hatedPeopleName = mMessage.frm.username
                    return True
                else:
                    return False
        return False
    ...
    def checkGogobotCmd(self, message):
        ...
        if 'gogoreload' in message:
            ...
            self.send(mMessage.getfrm, 'command received, reloading', message_type=mMessage.type)
        ...
        if 'gogoreset' in message:
            ...
            self.histFrom = []
            self.histMsg = []
            self.send(mMessage.getfrm, 'command received, history is now clean', message_type=mMessage.type)
        ...
        if 'gogotest' in message:
            self.send(mMessage.getfrm, '@NickJian', message_type=mMessage.type)
    ...
    def callback_message(self, message):
        ...
        # ###### pre-process #######
        message_string = message.body.lower()
        message_from = message.frm.nick

        if not message.frm or not message_string:
            print 'error ', message_from
            print 'error ', message_string
            return

        if message_from == 'gogobot':
            return
        ...
        # FIXME: this function should be fixed
